...
async function generateCasesWithAI(count = 4) {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: "user",
        content: `${count} adet case interview senaryosu üret. Her biri farklı bir sektörden ve zorluk seviyesinden olsun (easy/medium/hard karışık). Şu JSON şemasına birebir uy: {"cases": [{"title": "...", "industry": "...", "difficulty": "easy|medium|hard", "summary": "tek cümlelik özet", "prompt": "mülakatçıya verilecek 3-5 cümlelik detaylı case brief"}]}`,
      },
    ],
  });
...
async function seedCases() {
  ...
  const cases = await generateCasesWithAI(4);
